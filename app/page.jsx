...
export default function Home() {
  ...
  const handleSaveItem = () => {
    ...
    if (modalType === "add_inventory") {
      ...
      setInventory(
        inventory.map((i) => (i.id === editingItem.id ? editingItem : i)),
      );
    }
    closeModal();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-32 lg:pb-20">
      ...
      {modalType && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 lg:p-10 shadow-2xl overflow-y-auto max-h-[90vh]">
            {(modalType === "edit_inventory" ||
              modalType === "add_inventory") && (
              <div className="space-y-4">
                <h2 className="text-xl font-black uppercase text-blue-600 italic">
                  Product Master
                </h2>
                <input
                  className="w-full bg-slate-100 p-4 rounded-xl outline-none border-2 focus:border-blue-500 font-bold"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <input
                    ...
                    onChange={(e) =>
                      setEditingItem({ ...editingItem, hsn: e.target.value })
                    }
                  />
                  <select
                    className="w-full bg-slate-100 p-4 rounded-xl outline-none border-2 focus:border-blue-500 font-bold"
                    value={editingItem?.taxRate || 18}
                    onChange={(e) =>
                      setEditingItem({
                        ...editingItem,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <input
                    type="number"
                    className="w-full bg-slate-100 p-4 rounded-xl outline-none border-2 focus:border-blue-500 font-bold text-blue-600"
                    placeholder="Price"
                ...
                <button
                  ...
                  className="w-full bg-blue-600 text-white font-black py-4 rounded-xl shadow-lg mt-2 transition hover:bg-blue-700"
                >
                  Save Product
                </button>
              </div>
            ...
            {modalType === "confirm_delete" && (
              <div className="text-center p-4">
                ...
                <p className="text-slate-500 mb-6 text-sm">
                  Delete "{itemToDelete?.name}" permanently?
                </p>
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 bg-red-600 py-3 rounded-xl font-bold text-white"
                  >
                    Delete
                  </button>
                </div>
            ...
            <button
              onClick={closeModal}
              className="w-full mt-4 text-slate-400 font-bold text-[10px] uppercase"
            >
              Dismiss
            </button>
          </div>
      ...
      <nav className="bg-white border-b sticky top-0 z-50 shadow-sm px-4 lg:px-8 py-3 flex flex-col sm:flex-row items-center justify-between gap-4">
        <h1 className="text-xl font-black text-blue-600 italic">
          ERP_Lite{" "}
          <span className="text-[8px] not-italic text-slate-400 border px-1 rounded ml-1">
            PRO
          </span>
        </h1>
        <div className="flex bg-slate-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto">
          {["billing", "inventory", "history"].map((v) => (
            <button
              ...
              className={`flex-1 sm:flex-none px-4 lg:px-8 py-2 rounded-lg text-[10px] font-black uppercase transition-all whitespace-nowrap ${view === v ? "bg-white shadow-sm text-blue-600" : "text-slate-400"}`}
            >
              {v}
            </button>
          ))}
          ...
          <div className="grid lg:grid-cols-12 gap-6 lg:gap-10">
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-white p-4 lg:p-6 rounded-2xl lg:rounded-[2rem] border grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input
                  className="w-full bg-slate-50 p-4 rounded-xl outline-none border focus:border-blue-400 text-sm font-bold"
                  value={customer.name}
                  onChange={(e) =>
              ...
              {!isDone && (
                <div className="relative">
                  <input
                    ...
                    className="w-full p-4 lg:p-6 rounded-2xl border-2 outline-none focus:border-blue-600 shadow-sm text-lg"
                    value={searchTerm}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          >
            {[
              ...
              {
                label: "Stock Value (Ex. Tax)",
                value: `Rs.${inventory.reduce((s, i) => s + i.price * i.quantity, 0).toLocaleString("en-IN")}`,
                color: "#006400",
                small: true,
              },
            ].map((metric, idx) => (
              <div key={idx} className="win-groupbox">
                ...
                >
                  <span
                    style={{
                      fontSize: metric.small ? "16px" : "28px",
                      fontWeight: "bold",
          ...
          >
            ...
            <span style={{ fontSize: "11px", color: "#555" }}>
              Master Database — {inventory.length} record(s)
            </span>
          </div>

          <div
            className={`bg-white rounded-2xl border overflow-hidden ${isDone ? "opacity-50" : ""}`}
          >
                    ...
                    <tr key={item.id} className="border-b last:border-0">
                      <td className="p-4">
                        ...
                        <p className="text-[9px] text-slate-400">
                          ₹{item.price.toFixed(2)} + {item.taxRate}%
                        </p>
                      </td>
                      <td className="p-4">
                        <input
            ...
            {cart.length === 0 && (
              <p className="p-10 text-center text-slate-300 italic">
                Cart is empty
              </p>
            )}
          </div>
      ...
      {view === "history" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <div className="bg-slate-900 text-white p-6 lg:p-8 rounded-3xl lg:rounded-[2.5rem] shadow-xl">
              <div className="space-y-3 mb-6 text-[10px] uppercase font-bold tracking-widest">
                ...
                <div className="flex justify-between text-emerald-400 items-center">
                  ...
                  <input
                    type="number"
                    className="bg-slate-800 rounded px-2 py-1 w-12 text-right outline-none"
                    value={discount}
                    onChange={(e) =>
                      setDiscount(
                        Math.min(
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span>
                    ₹{(billingSummary.cgst + billingSummary.sgst).toFixed(2)}
                  </span>
                </div>
                <div className="h-px bg-white/10 my-2"></div>
                <h2 className="text-4xl lg:text-5xl font-black text-center py-2">
                  ₹{billingSummary.finalTotal.toFixed(0)}
                </h2>
              </div>
          ...
          <div className="win-statusbar">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-2xl border shadow-sm border-red-100">
                    <p className="text-red-400 text-[10px] font-black uppercase mb-1">
                      Low Stock
                    </p>
                  ...
                  <div className="bg-white p-6 rounded-2xl border shadow-sm">
                    <p className="text-emerald-400 text-[10px] font-black uppercase mb-1">
                      Stock Value
                    </p>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border">
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto bg-blue-600 text-white px-6 py-3 rounded-xl font-black text-sm"
                  >
                    ADD PRODUCT
                  </button>
                </div>
                      ...
                      <tbody>
                        {inventory.map((item) => (
                          ...
                          >
                            ...
                            <td className="p-4">
                              <span
                                className={`font-bold ${item.quantity < 10 ? "text-red-500" : ""}`}
                              >
                                {item.quantity}
                              </span>
                            </td>
            ...
            {view === "history" && (
              <div className="space-y-4">
                <h2 className="text-xl font-black uppercase text-slate-400 mb-6">
                  Recent Sales
                </h2>
                {history.map((bill) => (
                  <div
                    key={bill.billId}
                    className="bg-white p-5 rounded-2xl border shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                  >
                    <div>
                    ...
                    <div className="flex items-center justify-between w-full sm:w-auto gap-6">
                      <div className="text-right">
                        ...
                        <p className="text-xl font-black text-blue-600">
                          ₹{bill.finalTotal.toFixed(0)}
                        </p>
                      </div>
                      <button
                        onClick={() => generatePDF(bill)}
                        className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase"
                      >
                        PDF
                      </button>
                    </div>
                ...
                {history.length === 0 && (
                  <div className="p-20 text-center text-slate-300 italic border-2 border-dashed rounded-2xl">
                    No sales found.
                  </div>
                )}
              </div>
